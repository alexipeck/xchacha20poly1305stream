...
mod tests {
    ...
    fn test_basic_encryption_decryption() {
        ...
        let verified = verify!(&key, &nonce, &tag, |streamer| {
    ...
    fn test_different_keys_and_nonces() {
        ...
        let verify1 = verify!(&key1, &nonce1, &tag1, |streamer| {
            streamer.feed(&mut decryption1);
        });
        assert!(verify1);

        decrypt!(&key1, &nonce1, |streamer| {
            streamer.feed(&mut decryption1);
        });
        assert_eq!(decryption1, plaintext);

        let mut decryption2 = ciphertext2.clone();
        let verify2 = verify!(&key2, &nonce2, &tag2, |streamer| {
            streamer.feed(&mut decryption2);
        });
        assert!(verify2);

        decrypt!(&key2, &nonce2, |streamer| {
            streamer.feed(&mut decryption2);
        });
    ...
    fn test_with_associated_data() {
        ...
        let tag2 = encrypt!(&key, &nonce, |streamer| {
        ...
        let verify2 = verify!(&key, &nonce, &tag2, |streamer| {
            streamer.add_associated_data(&associated_data1);
            streamer.add_associated_data(&associated_data2);
            streamer.feed(&mut decrypted2);
        });
    ...
    fn test_wrong_order_decryption() {
        ...
        let mut authenticator = authenticate::Authenticator::new(&key, &nonce);
        authenticator.feed(&decrypt1);
        authenticator.feed(&decrypt2);
